...
cor(data[, sapply(data, is.numeric)])
    ...
    ## Plot AUROC
    perf_auroc<-performance(prediction(data.test$score,data.test$Flag),"auc")
    auroc<-as.numeric(perf_auroc@y.values)
    
    perf_plot<-performance(prediction(data.test$score,data.test$Flag),"tpr","fpr")
    ...
    perf_auroc1<-performance(prediction(data.test$score1,data.test$Flag),"auc")
    auroc1<-as.numeric(perf_auroc1@y.values)
    
    perf_plot1<-performance(prediction(data.test$score1,data.test$Flag),"tpr","fpr")
    ...
    fit3_fitForest.na<-as.data.frame(cbind(data.test$Flag,fit3_fitForest))
    ...
    nn <- neuralnet(Flag ~ ., data=data.train_1,
    ...
    nn_pred <- compute(nn, data.test_1[, -which(names(data.test_1) == "Flag")])
